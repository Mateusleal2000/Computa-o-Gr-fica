...
namespace display
{
    std::vector<int> scene(displayStructs::Viewport viewport, Eigen::Vector3d O, std::vector<std::shared_ptr<displayStructs::LightSource>> lightSources, std::vector<std::shared_ptr<Object>> objects)
    {
        ...
        for (int r = 0; r < viewport.nRows; r++)
        {
            ...
            for (int c = 0; c < viewport.nColumns; c++)
            {
                ...
                std::tuple<int, int, int> color = utils::traceRay(O, D, lightSources, objects);
